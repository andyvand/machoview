                 ...
                 length:(uint32_t)length
{
      ...
      switch ([CIE_augmentationStr characterAtIndex:strIndex])
      {
          ...
          // The size of the personality routine pointer is specified by the pointer encoding used.
        {
          ...
          [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                 ...
                                 :@"Personality Routine"
                                 :[self is64bit] == NO
                                    ? [self guessSymbolUsingEncoding:PR_encoding atOffset:(uint32_t)range.location withValue:(uint32_t &)PR_offset]
                                    : [self guessSymbol64UsingEncoding:PR_encoding atOffset:(uint32_t)range.location withValue:PR_offset]];
        } break;
  ...
  while (NSMaxRange(range) - location < length)
  {
    ...
    uint64_t FDE_CIEpointer = ([self is64bit] == NO 
                                ? [self fileOffsetToRVA:(uint32_t)range.location]
                                : [self fileOffsetToRVA64:(uint32_t)range.location]) - FDE_CIEvalue;
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                           ...
                           :@"PC Begin"
                           :[self is64bit] == NO
                              ? (symbolName = [self guessSymbolUsingEncoding:Pointer_encoding atOffset:(uint32_t)range.location withValue:(uint32_t &)PCBegin_addr])
                              : (symbolName = [self guessSymbol64UsingEncoding:Pointer_encoding atOffset:(uint32_t)range.location withValue:PCBegin_addr])];
    ...
    if ([CIE_augmentationStr rangeOfString:@"z"].location != NSNotFound)
    {
      ...
      if (FDE_augmentationLength > 0) // LSDA is present
      {
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                               ...
                               :@"LSDA"
                               :[self is64bit] == NO 
                                  ? [self guessSymbolUsingEncoding:LSDA_encoding atOffset:(uint32_t)range.location withValue:(uint32_t &)LSDA_addr]
                                  : [self guessSymbol64UsingEncoding:LSDA_encoding atOffset:(uint32_t)range.location withValue:LSDA_addr]];
              ...
              
{
  ...
  if (typeTableFormat != DW_EH_PE_omit)
  {
    ...
    typeTableBaseLocation = (uint32_t)(NSMaxRange(range) + typeTableBaseOffset);
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                           :lastReadHex
                           :@"Type Table Base"
  ...
  if (typeTableFormat != DW_EH_PE_omit)
  {
    ...
    while (actions.empty() == false)
    {
      ...
      if (index > 0)
      {
        typeIndexes.insert((uint32_t)index);
      }
      else if (index < 0)
      {
        exceptionSpecs.insert((uint32_t)index);
      }
    ...
    for (IndexSet::iterator iter = exceptionSpecs.begin(); iter != exceptionSpecs.end(); ++iter)
    {
      ...
      for (;;)
      {
        index = (int32_t)[dataController read_sleb128:range lastReadHex:&lastReadHex];
        if (index == 0)
        {
          break;
        }
    ...
    for (IndexSet::reverse_iterator iter = typeIndexes.rbegin(); iter != typeIndexes.rend(); ++iter)
    {
      ...
      [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                              ...
                              :@"Type Info"
                              :[self is64bit] == NO
                                ? [self guessSymbolUsingEncoding:typeTableFormat atOffset:(uint32_t)range.location withValue:(uint32_t &)typeInfo]
                                : [self guessSymbol64UsingEncoding:typeTableFormat atOffset:(uint32_t)range.location withValue:typeInfo]];
    }
                                ...
                                header:(struct unwind_info_section_header const *)unwind_info_section_header
{
  ...
  [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                         ...
                         :[self is64bit] == NO 
                            ? [self findSymbolAtRVA:[self fileOffsetToRVA:(uint32_t)range.location] + commonEncodingsArraySectionOffset]
                            : [self findSymbolAtRVA64:(uint32_t)([self fileOffsetToRVA64:(uint32_t)range.location] + commonEncodingsArraySectionOffset)]];
  ...
  [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                         ...
                         :[self is64bit] == NO 
                            ? [self findSymbolAtRVA:[self fileOffsetToRVA:(uint32_t)range.location] + personalityArraySectionOffset]
                            : [self findSymbolAtRVA64:[self fileOffsetToRVA64:(uint32_t)range.location] + personalityArraySectionOffset]];
  ...
  [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                         ...
                         :[self is64bit] == NO 
                            ? [self findSymbolAtRVA:[self fileOffsetToRVA:(uint32_t)range.location] + indexSectionOffset]
                            : [self findSymbolAtRVA64:[self fileOffsetToRVA64:(uint32_t)range.location] + indexSectionOffset]];
