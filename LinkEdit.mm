    ...
    if (relocation_info->r_extern)
    {
      ...
      else if (prev_relocation_info)
      {
        ...
        if (relocLength == sizeof(uint32_t))
        {
          ...
          uint32_t relocValue = (uint32_t)((uint64_t)(nlist_64->n_value - prev_nlist_64->n_value) + relocAddend);
                           ...
                           length:(uint32_t)length
{
  ...
  {
    MATCH_STRUCT(dylib_reference, (uint32_t)NSMaxRange(range))
                    ...
                    length:(uint32_t)length
{
  ...
  while (NSMaxRange(range) < location + length)
  {
    MATCH_STRUCT(dylib_table_of_contents, (uint32_t)NSMaxRange(range))
                        ...
                        length:(uint32_t)length
{
  ...
  while (NSMaxRange(range) < location + length)
  {
    MATCH_STRUCT(dylib_module, (uint32_t)NSMaxRange(range))
                          ...
                          length:(uint32_t)length
{
  ...
  while (NSMaxRange(range) < location + length)
  {
    MATCH_STRUCT(dylib_module_64, (uint32_t)NSMaxRange(range))
                               ...
                               index:(uint32_t)index
{
  ...
  while (NSMaxRange(range) < location + length)
  {
    MATCH_STRUCT(twolevel_hint, (uint32_t)NSMaxRange(range))
    ...
    do
    {
      ...
      [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                             ...
                             :[NSString stringWithFormat:@"%@ %@",
                               [self is64bit] == NO ? [self findSectionContainsRVA:(uint32_t)address] : [self findSectionContainsRVA64:(uint32_t)address],
                                  ...
                                  length:(uint32_t)length
{
  ...
  while (NSMaxRange(range) < location + length)
  {
    MATCH_STRUCT(data_in_code_entry, (uint32_t)NSMaxRange(range))
