  ...
  while (NSMaxRange(range) < location + length && isDone == NO)
  {
    ...
    switch (opcode) 
    {
      ...
      case REBASE_OPCODE_DO_REBASE_IMM_TIMES: 
      {
        ...
        doRebaseLocation = (uint32_t)NSMaxRange(range);
      ...
      case REBASE_OPCODE_DO_REBASE_ULEB_TIMES: 
      {
        ...
        uint32_t startNextRebase = (uint32_t)NSMaxRange(range);
  ...
  while (NSMaxRange(range) < location + length && isDone == NO)
  {
    ...
    switch (opcode) 
    {
      case BIND_OPCODE_DONE:
        ...
        doBindLocation = (uint32_t)NSMaxRange(range);
      ...
      case BIND_OPCODE_DO_BIND_ADD_ADDR_ULEB: 
      {
        ...
        uint32_t startNextBind = (uint32_t)NSMaxRange(range);
      ...
      exportLocation:(uint32_t &)exportLocation
{
  ...
  if (terminalSize != 0) 
  {
    ...
    if ((flags & EXPORT_SYMBOL_FLAGS_KIND_MASK) == EXPORT_SYMBOL_FLAGS_KIND_REGULAR)      [node.details insertRowWithOffset:(uint32_t)range.location:@"":@"":@"00":@"EXPORT_SYMBOL_FLAGS_KIND_REGULAR"];
    if ((flags & EXPORT_SYMBOL_FLAGS_KIND_MASK) == EXPORT_SYMBOL_FLAGS_KIND_THREAD_LOCAL) [node.details insertRowWithOffset:(uint32_t)range.location:@"":@"":@"01":@"EXPORT_SYMBOL_FLAGS_KIND_THREAD_LOCAL"];
    if (flags & EXPORT_SYMBOL_FLAGS_WEAK_DEFINITION)                                      [node.details insertRowWithOffset:(uint32_t)range.location:@"":@"":@"04":@"EXPORT_SYMBOL_FLAGS_WEAK_DEFINITION"];
    if (flags & EXPORT_SYMBOL_FLAGS_REEXPORT)                                             [node.details insertRowWithOffset:(uint32_t)range.location:@"":@"":@"08":@"EXPORT_SYMBOL_FLAGS_REEXPORT"];
    if (flags & EXPORT_SYMBOL_FLAGS_STUB_AND_RESOLVER)                                    [node.details insertRowWithOffset:(uint32_t)range.location:@"":@"":@"10":@"EXPORT_SYMBOL_FLAGS_STUB_AND_RESOLVER"];
