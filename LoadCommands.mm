              ...
              thread_command:(struct thread_command const *)thread_command
{
  ...
  {
    MATCH_STRUCT(x86_thread_state,(uint32_t)NSMaxRange(range))
  ...
  else if (mach_header->cputype == CPU_TYPE_ARM)
  {
    ...
    MATCH_STRUCT(arm_thread_state,(uint32_t)NSMaxRange(range))
               ...
               build_version_command:(struct build_version_command const *)build_version_command
{
    ...
    NSRange range = NSMakeRange(location,0);
    NSString * lastReadHex;
    NSUInteger currentTool = 0;
    struct build_tool_version *build_tool_version = (struct build_tool_version *)((uint64_t)build_version_command + sizeof(struct build_version_command));
    ...
    case PLATFORM_TVOS:
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Platform"
                                   :[NSString stringWithFormat:@"AppleTV OS"]];
            break;
    ...
    case PLATFORM_WATCHOS:
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Platform"
                                   :[NSString stringWithFormat:@"Watch OS"]];
            break;
    ...
    case PLATFORM_BRIDGEOS:
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Platform"
                                   :[NSString stringWithFormat:@"Bridge OS"]];
            break;
    ...
    case PLATFORM_IOSMAC:
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Platform"
                                   :[NSString stringWithFormat:@"iOS Mac"]];
            break;
    ...
    case PLATFORM_IOSSIMULATOR:
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Platform"
                                   :[NSString stringWithFormat:@"iOS Simulator"]];
            break;
    ...
    case PLATFORM_TVOSSIMULATOR:
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Platform"
                                   :[NSString stringWithFormat:@"AppleTV OS Simulator"]];
            break;
    ...
    case PLATFORM_WATCHOSSIMULATOR:
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Platform"
                                   :[NSString stringWithFormat:@"Watch OS Simulator"]];
            break;
    ...
    default:
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Platform"
                                   :[NSString stringWithFormat:@"%u: *** Unknown ***", build_version_command->platform]];
            break;
    }
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                           :lastReadHex
                           :@"SDK Version"
                           :[NSString stringWithFormat:@"%u.%u.%u",                              (build_version_command->sdk >> 16),
    ...
    while (currentTool < build_version_command->ntools)
    {
        [dataController read_uint32:range lastReadHex:&lastReadHex];
        switch (build_tool_version[currentTool].tool)
        {
            case TOOL_CLANG:
                [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                       :lastReadHex
                                       :[NSString stringWithFormat:@"Tool %u name", (unsigned int)currentTool+1]
                                       :[NSString stringWithFormat:@"Clang"]];
                break;
            ...
            case TOOL_SWIFT:
                [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                       :lastReadHex
                                       :[NSString stringWithFormat:@"Tool %u name", (unsigned int)currentTool+1]
                                       :[NSString stringWithFormat:@"Swift"]];
                break;
            ...
            default:
                [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                       :lastReadHex
                                       :[NSString stringWithFormat:@"Tool %u name", (unsigned int)currentTool+1]
                                       :[NSString stringWithFormat:@"%u: *** Unknown ***", build_tool_version[currentTool].tool]];
                break;
        }
        ...
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                               :lastReadHex
                               :[NSString stringWithFormat:@"Tool %u version", (unsigned int)(currentTool+1)]
                               :[NSString stringWithFormat:@"%u.%u.%u",                          (build_tool_version[currentTool].version >> 16),
                                 ((build_tool_version[currentTool].version >> 8) & 0xff),
                                 (build_tool_version[currentTool].version & 0xff)]];
...
- (MVNode *)createNoteCommand:(MVNode *)parent
                           caption:(NSString *)caption
                          location:(uint32_t)location
             note_command:(struct note_command const *)note_command
{
    ...
    NSRange noteData = { note_command->offset, note_command->size };
    [dataController read_bytes:noteData length:note_command->size lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", noteData.location]
                           :lastReadHex
                           :@"Note data"
                           :lastReadHex];
                         ...
                         command:(uint32_t)command
{
  ...
  switch (command)
  {
    ...
    case LC_SEGMENT_64:
    {
      ...
      // preserv segment RVA/size for offset lookup
      segmentInfo[(uint32_t)(segment_command_64->fileoff + imageOffset)] = make_pair(segment_command_64->vmaddr, segment_command_64->vmsize);
    ...
    case LC_BUILD_VERSION:
    {
        ...
        node = [self createBuildVersionNode:parent caption:caption location:location build_version_command:build_version_command];
    } break;
    ...
    case LC_NOTE:
    {
        ...
        node = [self createNoteCommand:parent caption:caption location:location note_command:note_command];
    } break;
