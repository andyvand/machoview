...
- (void)handleDataTreeChanged:(NSNotification *)notification
{
  if (notification.object == dataController)
  {
    dispatch_async(dispatch_get_main_queue(), ^
    {
      ...
      {
        [leftView reloadItem:self.dataController.rootNode reloadChildren:YES];
      }
    });
  }
}
...
- (void)handleThreadStateChanged:(NSNotification *)notification
{
  if (notification.object == dataController)
  {
    ...
    if ([threadState isEqualToString:MVStatusTaskStarted] == YES)
    {
      ...
      if (atomic_fetch_add(&threadCount, 1) == 1)
#else
    ...
    {
#if MAC_OS_X_VERSION_MIN_REQUIRED >= 101200
      if (atomic_fetch_sub(&threadCount, 1) == 1)
#else
...
- (BOOL)readFromURL:(NSURL *)absoluteURL ofType:(NSString *)typeName error:(NSError **)outError
{
  ...
  @try 
  {
    [dataController createLayouts:dataController.rootNode location:0 length:(uint32_t)(dataController.fileData).length];
  }
